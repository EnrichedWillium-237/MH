...
string iov = "PbPb_262548_262799";
// string iov = "PbPb_262800_263230";
// string iov = "PbPb_263231_263359";
// string iov = "PbPb_263360_263379";
// string iov = "PbPb_263380_263614";
// string iov = "PbPb_263615_263757";
...
void GetVN( string name="N1SUB3", string tag="useTight", double mineta = -2.4, double maxeta = 2.4, bool override = false ) {
    ...
    EtaMin = mineta;
    EtaMax = maxeta;
    stag = "_"+iov+"_"+Form("%0.1f_%0.1f",EtaMin,EtaMax);
    rootFile = "";
    isTight = false;
    isTightB = false;
    isLoose = false;
    if (tag == "useTight") {
        ...
        isTightB = false;
        isLoose = false;
        rootFile = Form("../../%s.root",iov.data());
        fin = new TFile(Form("../../%s.root",iov.data()),"read");
    } else if (tag == "useTightB") {
        isTight = false;
        isTightB = true;
        isLoose = false;
        rootFile = "../MH_tightB.root";
        fin = new TFile("../MH_tightB.root","read");
    } else {
        isTight = false;
        isTightB = false;
        isLoose = true;
    ...
    fout = new TFile(Form("results/results%s/%s.root",stag.data(),name.data()),"recreate");

    for (int bin = 0; bin<=cbins; bin++) {
        // if(bin!=5) continue;
        GetVNCreate(en,bin);

        TDirectory * tdcent = (TDirectory *) fout->mkdir(Form("%d_%d",cmin[bin],cmax[bin]));
        tdcent->cd();
