...
void q1q2( string rootfile = "../MH.root", string Q2 = "track222", int cmin = 25, int cmax = 30 ) {

    TH1::SetDefaultSumw2();
    TH2::SetDefaultSumw2();
    string tag = "";
    if (rootfile == "../MH.root") tag+="default";
    if (rootfile == "MH_AMPT.root") tag+="AMPT";
    ...
    TPaveText * tx0 = new TPaveText(0.23, 0.84, 0.39, 0.90, "NDC");
    SetTPaveTxt(tx0, 22);
    tx0->AddText(Form("%d - %d%%",cmin,cmax));
    tx0->Draw();

    c0->Print(Form("q1q2plots/q1q2corr_%s_%d_%d.pdf",tag.data(),cmin,cmax),"pdf");
